    ...
    public class AngleTypeConverter : TypeConverter
    {
            ...
            object value)
        {
            ...
            if (value is double ||
                ...
                value is ulong)
            {
                return Angle.FromDegrees(Convert.ToDouble(value));
            }
            ...
            Type destinationType)
        {
            if (value is Angle)
            {
                if (destinationType == typeof(string))
                {
                    return ((Angle)value).ToString();
                }

                if (destinationType == typeof(double))
                {
                    return Convert.ToDouble(((Angle)value).TotalDegrees);
                }

                if (destinationType == typeof(float))
                {
                    return Convert.ToSingle(((Angle)value).TotalDegrees);
                }

                if (destinationType == typeof(decimal))
                {
                    return Convert.ToDecimal(((Angle)value).TotalDegrees);
                }

                if (destinationType == typeof(short))
                {
                    return Convert.ToInt16(((Angle)value).TotalDegrees);
                }

                if (destinationType == typeof(int))
                {
                    return Convert.ToInt32(((Angle)value).TotalDegrees);
                }

                if (destinationType == typeof(long))
                {
                    return Convert.ToInt64(((Angle)value).TotalDegrees);
                }

                if (destinationType == typeof(uint))
                {
                    return Convert.ToUInt32(((Angle)value).TotalDegrees);
                }

                if (destinationType == typeof(ulong))
                {
                    return Convert.ToUInt64(((Angle)value).TotalDegrees);
                }

                if (destinationType == typeof(ushort))
                {
                    return Convert.ToUInt16(((Angle)value).TotalDegrees);
                }
            }
