namespace More.Types
{
    ...
    /// <summary>
    /// Latitude converter class.
    /// </summary>
    public class LatitudeTypeConverter : TypeConverter
    {
        /// <summary>
            ...
            object value)
        {
            if (value is string)
            {
                return Latitude.Parse((string)value);
            }
                ...
                value is ulong)
            {
                return Latitude.FromDegrees(Convert.ToDouble(value));
            }
            ...
            Type destinationType)
        {
            if (value is Latitude)
            {
                ...
                {
                    return Convert.ToDouble(((Latitude)value).TotalDegrees);
                }

                if (destinationType == typeof(float))
                {
                    return Convert.ToSingle(((Latitude)value).TotalDegrees);
                }

                if (destinationType == typeof(decimal))
                {
                    return Convert.ToDecimal(((Latitude)value).TotalDegrees);
                }

                if (destinationType == typeof(short))
                {
                    return Convert.ToInt16(((Latitude)value).TotalDegrees);
                }

                if (destinationType == typeof(int))
                {
                    return Convert.ToInt32(((Latitude)value).TotalDegrees);
                }

                if (destinationType == typeof(long))
                {
                    return Convert.ToInt64(((Latitude)value).TotalDegrees);
                }

                if (destinationType == typeof(uint))
                {
                    return Convert.ToUInt32(((Latitude)value).TotalDegrees);
                }

                if (destinationType == typeof(ulong))
                {
                    return Convert.ToUInt64(((Latitude)value).TotalDegrees);
                }

                if (destinationType == typeof(ushort))
                {
                    return Convert.ToUInt16(((Latitude)value).TotalDegrees);
                }
            }
