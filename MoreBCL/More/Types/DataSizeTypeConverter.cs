namespace More.Types
{
    ...
    public class DataSizeTypeConverter : TypeConverter
    {
            ...
            object value)
        {
            if (value is string)
            {
                return DataSize.Parse((string)value);
            }
            ...
            if (value is double ||
                ...
                value is ulong)
            {
                return DataSize.FromBytes(Convert.ToDouble(value));
            }
            ...
            Type destinationType)
        {
            if (value is DataSize)
            {
                if (destinationType == typeof(string))
                {
                    return ((DataSize)value).ToString();
                }

                if (destinationType == typeof(double))
                {
                    return Convert.ToDouble(((DataSize)value).Bytes);
                }

                if (destinationType == typeof(float))
                {
                    return Convert.ToSingle(((DataSize)value).Bytes);
                }

                if (destinationType == typeof(decimal))
                {
                    return Convert.ToDecimal(((DataSize)value).Bytes);
                }

                if (destinationType == typeof(short))
                {
                    return Convert.ToInt16(((DataSize)value).Bytes);
                }

                if (destinationType == typeof(int))
                {
                    return Convert.ToInt32(((DataSize)value).Bytes);
                }

                if (destinationType == typeof(long))
                {
                    return Convert.ToInt64(((DataSize)value).Bytes);
                }

                if (destinationType == typeof(uint))
                {
                    return Convert.ToUInt32(((DataSize)value).Bytes);
                }

                if (destinationType == typeof(ulong))
                {
                    return Convert.ToUInt64(((DataSize)value).Bytes);
                }

                if (destinationType == typeof(ushort))
                {
                    return Convert.ToUInt16(((DataSize)value).Bytes);
                }
            }
